...
export default function AsyncApp() {
  ...
  const [currentColours, setCurrentColours] = useState(
    cyclePresets[currentCycle].colours
  );
  ...
  useEffect(() => {
    currentCanvasStateRef.current = currentCanvasState;
  }, [currentCanvasState]);
  ...
  function setInput(changeToDraw) {
    if (changeToDraw) {
      ...
      document
        .getElementById("draw-input-select")
        .classList.add("ActiveInputButton");
      ...
      document
        .getElementById("text-input-select")
        .classList.remove("ActiveInputButton");
    } else {
  ...
  function updateCanvasBrush() {
    drawingCanvasRef.current.freeDrawingBrush.color =
      currentColours[selectedColourRef.current];
  ...
  function saveCanvasState() {
    ...
    if (isDrawingRef.current) {
      ...
      if (newUndoStack.length === maxUndo) {
        newUndoStack.shift();
      }
  ...
  function undoDrawing() {
    ...
    if (undoStack.length > 0) {
      ...
      newRedoStack.splice(
        newRedoStack.length,
        0,
        drawingCanvasRef.current.toDatalessJSON()
      );
  ...
  function toggleFocus() {
    ...
    if (!focusMode) {
      ...
      document
        .getElementById("settings-focus-button")
  ...
  function getCustomEraserCursor() {
    const circle = `<svg height="${brushSize}"
    ...
    return `url(data:image/svg+xml;base64,${window.btoa(circle)}) ${
      brushSize / 2
    } ${brushSize / 2}, move`;
  }
